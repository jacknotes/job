...
| 子句       | 作用                          | 是否影响评分（`_score`） | 示例场景                       |
| ---------- | ----------------------------- | ------------------------ | ------------------------------ |
| `must`     | 必须满足（AND）               | ✅ 是                     | `name` 必须包含 “手机”         |
| `filter`   | 必须满足（AND），但**不评分** | ❌ 否（性能更高）         | `price` 在 1000~3000 之间      |
| `should`   | 至少满足一个（OR）            | ✅ 是                     | `category` 为 “电子” 或 “数码” |
| `must_not` | 必须不满足（NOT）             | ❌ 否                     | 排除 `brand: "A"`              |
...
| 查询类型              | 适用场景                     | 示例                                                 |
| --------------------- | ---------------------------- | ---------------------------------------------------- |
| `match`               | 全文检索（分词后匹配）       | `"match": { "message": "error" }`                    |
| `match_phrase`        | 精确短语匹配                 | `"match_phrase": { "message": "文件删除失败" }`      |
| `term` / `terms`      | 精确值匹配（不分词）         | `"term": { "status": "active" }`                     |
...
6      ],
7      "filter": [
8        { "range": { "price": { "gte": 1000, "lte": 3000 } } },
9        { "term": { "status": "in_stock" } }
10      ],
11      "should": [
12        { "term": { "category": "电子产品" } },
13        { "term": { "category": "数码配件" } }
14      ],
15      "must_not": [
16        { "term": { "brand": "A" } }
17      ],
...
- 对**不需要相关性评分**的条件（如状态、ID、时间范围），优先用 `filter`。
- 避免在 `should` 中混用高成本查询（如 `wildcard`、`regexp`）。
- 使用 `constant_score` 包装纯过滤查询，可进一步提升性能：
