...
increment_restore(){
	...
	for i in `seq 1 ${RESTORE_DATABASE_NUMBER}`;do
		...
		if [ `ls ${DATADIR} | grep ${CURRENT_RESTORE_DATABASE_FILE}` ];then 
			...
			echo "`eval ${DATETIME}`: in ${DATADIR} directory relate to ${CURRENT_RESTORE_DATABASE_FILE} file not exists,"
			exit 6
		fi
		...
		if [ -e "${DATADIR}/${CURRENT_RESTORE_DATABASE_NAME}-${CURRENT_EXCUTE_MASTER_INDEX}-${CURRENT_RESTORE_DATABASE_POSITION}.sql" ];then
			echo "`eval ${DATETIME}`: ${DATADIR}/${CURRENT_RESTORE_DATABASE_NAME}-${CURRENT_EXCUTE_MASTER_INDEX}-${CURRENT_RESTORE_DATABASE_POSITION}.sql already exists."
				...
				ln -s ${DATADIR}/${OLD_RESTORE_DATABASE_NAME}-${OLD_EXCUTE_MASTER_INDEX}-${OLD_RESTORE_DATABASE_POSITION}.sql ${DATADIR}/${CURRENT_RESTORE_DATABASE_NAME}-${CURRENT_EXCUTE_MASTER_INDEX}-${CURRENT_RESTORE_DATABASE_POSITION}.sql
	...
	for i in `seq 1 ${RESTORE_DATABASE_NUMBER}`;do
		...
		CURRENT_RESTORE_DATABASE_POSITION=`echo ${CURRENT_RESTORE_DATABASE_INFO} | awk '{print $3}'`
		if [ ! -L "${DATADIR}/${CURRENT_RESTORE_DATABASE_NAME}-${CURRENT_EXCUTE_MASTER_INDEX}-${CURRENT_RESTORE_DATABASE_POSITION}.sql" ];then
			...
			echo "`eval ${DATETIME}`: ${DATADIR}/${CURRENT_RESTORE_DATABASE_NAME}-${CURRENT_EXCUTE_MASTER_INDEX}-${CURRENT_RESTORE_DATABASE_POSITION}.sql is soft link, so already execute."
		fi
		echo '---'
	done
