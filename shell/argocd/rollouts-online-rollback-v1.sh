...
ARGO_ROLLOUT_PROJECT_NAME=(`kubectl argo rollouts list rollout -A | awk -F ' ' '{if($4=="Paused") print $1,$2}'`)
...
online(){
	...
	if [ $? == 0 ];then
		for i in `seq 1 ${group_number}`;do
			...
			# promote application
			kubectl argo rollouts promote ${ARGO_ROLLOUT_PROJECT_NAME[${sub_group2}]} -n ${ARGO_ROLLOUT_PROJECT_NAME[${sub_group1}]}
		done
	fi
}
