...
for i in `eval ${IndexList}`;do
	...
	curl -s -u ${Username}:${Password} -XPOST "${ESAddress}/_reindex" -H 'Content-Type: application/json' -d'{  "source": {    "index": "'${i}'", "size": 100  },  "dest": {    "index": "'${i}${Suffix}'"  }}' | grep 'acknowledged":true' &> /dev/null
