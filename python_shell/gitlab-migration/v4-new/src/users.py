...
class Users(object):
	...
	def get(self):
		...
		print(str(datetime.now()), '[INFO] Total users: %d' % len(users))
	...
	def inserts(self, users):
		...
		size = len(new_users)
		print(str(datetime.now()), '[INFO] Create new user: %d' % (size - 1))
		print(str(datetime.now()), '[INFO] Total new user: %d' % size)
