...
class Repositories(object):
	def __init__(self, cfg, source_projects):
		...
		# self.source_api = 'http://%s/api/v4/projects/%s/repository/tree'
	...
	def run(self):
		...
		for i,project in enumerate(self.source_projects):
			# 加了if语句，主要用于调试时使用
			if i+1 >= 0:
				print(str(datetime.now()), "[INFO] number %s" % str(i+1))
				...
				if request_status.status_code == 404:
					print(str(datetime.now()), "[INFO] project %s content is Null" % project['path_with_namespace'])
					continue
	...
	def push(self, uri, to_path):
		...
		print(str(datetime.now()), '[INFO] Clone:', source_url)
		repo = Repo.clone_from(url = source_url, to_path = to_path, bare = True)
		print(str(datetime.now()), '[INFO] Push:', target_url)
		gitlab = repo.create_remote('gitlab', target_url)
		print(str(datetime.now()), '[INFO] push all')
		...
		print(str(datetime.now()), '[INFO] push tags')
